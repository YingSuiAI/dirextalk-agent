...
[[ $(jq -r .image_version "$release_file") == 1.1.3 ]]
...
name=dirextalk-worker-$flavor-1-1-3
resource_tags=$(jq -n --arg flavor "$flavor" '{DirextalkWorkerImageSchema:"1",DirextalkWorkerImageFlavor:$flavor,DirextalkWorkerImageVersion:"1.1.3",DirextalkPiVersion:"0.84.4",DirextalkImageTested:"true"}')
...
jq -n --arg name "$name-install" --rawfile data "$output_dir/install.yaml" \
  --argjson tags "$resource_tags" '{name:$name,semanticVersion:"1.1.3",description:"Dirextalk Worker install component 1.1.3",platform:"Linux",data:$data,tags:$tags}' \
  >"$output_dir/build-component.json"
jq -n --arg name "$name-test" --rawfile data "$output_dir/test.yaml" \
  --argjson tags "$resource_tags" '{name:$name,semanticVersion:"1.1.3",description:"Dirextalk Worker test component 1.1.3",platform:"Linux",data:$data,tags:$tags}' \
  >"$output_dir/test-component.json"
jq -n --arg name "$name-plugin" --rawfile data "$output_dir/plugin.yaml" \
  --argjson tags "$resource_tags" '{name:$name,semanticVersion:"1.1.3",description:"Dirextalk Worker reviewed Pi plugin component 1.1.3",platform:"Linux",data:$data,tags:$tags}' \
...
jq -n --arg name "$name-infrastructure" --arg profile "$instance_profile" --arg type "$build_type" \
  ...
  '{name:$name,description:"Dirextalk Worker Image Builder infrastructure 1.1.3",instanceProfileName:$profile,instanceTypes:[$type],subnetId:$subnet,securityGroupIds:[$sg],terminateInstanceOnFailure:true,instanceMetadataOptions:{httpTokens:"required",httpPutResponseHopLimit:1},tags:$tags}' \
...
jq -n --arg name "$name-distribution" --arg account "$account_id" --arg flavor "$flavor" --argjson regions "$distribution_regions_json" --argjson resource_tags "$resource_tags" --argjson ami_tags "$ami_tags" \
  '{name:$name,description:"Dirextalk Worker public allowlisted multi-Region distribution 1.1.3",distributions:[$regions[] as $region|{region:$region,amiDistributionConfiguration:{name:("dirextalk-worker-"+$flavor+"-1.1.3-"+$region+"-{{ imagebuilder:buildDate }}"),description:"Public Dirextalk Worker AMI 1.1.3",launchPermission:{userGroups:["all"]},amiTags:$ami_tags},ssmParameterConfigurations:[{amiAccountId:$account,parameterName:("/dirextalk/worker-images/v1/"+$flavor+"/candidate"),dataType:"aws:ec2:image"}]}],tags:$resource_tags}' \
