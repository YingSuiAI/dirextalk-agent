...
for flavor in cpu gpu; do
  ...
  jq -e --arg flavor "$flavor" '.distributions[0].amiDistributionConfiguration.amiTags.DirextalkWorkerImageSchema == "1" and .distributions[0].amiDistributionConfiguration.amiTags.DirextalkWorkerImageFlavor == $flavor and .distributions[0].amiDistributionConfiguration.amiTags.DirextalkWorkerImageVersion == "1.1.4" and .distributions[0].amiDistributionConfiguration.amiTags.DirextalkPiVersion == "0.84.4" and .distributions[0].amiDistributionConfiguration.amiTags.DirextalkImageTested == "true"' "$work/$flavor/distribution.json" >/dev/null
  grep -Fq 'exec /opt/dirextalk-worker/pi/0.84.4/pi "$@"' "$work/$flavor/install.yaml"
  grep -Fq 'git clone --no-local --branch main' "$work/$flavor/test.yaml"
  if [[ $flavor == gpu ]]; then
