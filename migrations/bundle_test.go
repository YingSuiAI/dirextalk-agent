...
func TestBundleContainsCoreV1Migrations(t *testing.T) {
	...
	deepSeekDialect := Ordered()[31]
	for _, needle := range []string{"core_model_profiles_request_dialect_check", "deepseek_dsml_v4"} {
		if deepSeekDialect.Version != 32 || !bytes.Contains(deepSeekDialect.Script, []byte(needle)) {
			t.Fatalf("DeepSeek request dialect migration missing %q", needle)
		}
	}
	...
	toolCallFormatFinalization := Ordered()[28]
	for _, needle := range []string{"core_conversation_turn_finalizations_reason_check", "model_tool_call_format_invalid"} {
		if toolCallFormatFinalization.Version != 29 || !bytes.Contains(toolCallFormatFinalization.Script, []byte(needle)) {
			t.Fatalf("model tool-call format finalization migration missing %q", needle)
		}
	}
	turnSteerSupersedesModel := Ordered()[30]
	for _, needle := range []string{"superseded", "turn.dispatch_state <> 'dispatched'", "conversation.title=''"} {
		if turnSteerSupersedesModel.Version != 31 || !bytes.Contains(turnSteerSupersedesModel.Script, []byte(needle)) {
			t.Fatalf("turn steer superseded-model migration missing %q", needle)
		}
	}
	migration := Ordered()[0]
