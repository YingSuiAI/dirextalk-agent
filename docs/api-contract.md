...
`Chat` and `StreamChat` bind the UUID idempotency key to owner, conversation, input, requested memory mode, and expected conversation revision in the authenticated caller namespace. After the request lease is fenced, the coordinator persists the effective memory mode before model or tool execution; disabling memory is sticky across recovery. A stateless completion has revision zero and creates no conversation row. Persistent requests acquire the owner/conversation execution lock, reload the current revision inside that lock, and rebase an older expected revision upward before any model or tool call. A caller revision ahead of Central or a missing expected conversation still returns `ABORTED`. The lock remains held through the atomic conversation/response commit, so foreground Chat and background completion cannot execute against the same revision concurrently. An exact completed retry returns the original versioned response without loading current model configuration or re-running model/tools. A changed payload returns `ALREADY_EXISTS`. Internal `SynthesizeTeamCompletion` accepts only owner plus canonical source event ID, derives a deterministic runtime request identity, requires an existing persistent conversation, reloads verified Team completion facts, and commits a model-generated assistant reply through the same ledger. Only this exact trusted observation shape excludes expected revision from its immutable request digest, allowing the event retry loop to reacquire the latest conversation after a conflict; ordinary request digests remain bound to the caller's original revision even when execution is safely rebased after claim. `GetConversationState` exposes only existence and authoritative revision for Message Server reconciliation; it does not expose conversation text.
