...
Optional `AGENT_SEARCH_PROFILES_FILE` applies the same ownership rule to search configuration. Runtime capabilities expose only sorted public profile IDs. Runtime config stores a catalog-resolved provider, exact reviewed official endpoint, mounted-secret reference, and bounded result/timeout settings, while public read-back removes the secret reference. Updates preserve every unrelated runtime field and use the same owner/revision/idempotency fence. A selected profile automatically enables the query-only native `web_search` tool. Its typed adapter re-resolves the persisted profile against the immutable catalog before credential access, resolves the mounted key only for one invocation, disables redirects and environment proxies, and returns a bounded provider-neutral result after URL validation and text redaction. Cloud Dialogue requests deliberately remove the Search Profile and retain their narrower fixed tool set.

`TaskService.GetTaskOverview` returns one owner-scoped, read-only snapshot containing exact status-pair counts and a bounded recent-task projection. PostgreSQL derives the aggregate and recent rows inside one repeatable-read transaction. Message Server independently recomputes total, active, approval, running, completed, and attention counters before exposing the snapshot to Flutter, so the dashboard does not infer authoritative task state from cached chat messages or a sequence of list calls.
...
`Chat` and `StreamChat` run through the same native Eino ReAct engine. Runtime request and tool-call ledgers are caller-scoped and lease-fenced. The coordinator binds the effective memory mode before execution, with disabled persistence remaining sticky after lease recovery; stateless requests never create a conversation row. Every model round is bounded by the catalog profile's context window and preserves the system policy, newest user input, and complete tool-call/result groups. Persistent turns for one owner/conversation are serialized across Agent processes by a PostgreSQL advisory lock held from revision reconciliation through the final commit. Once the lock is acquired, the coordinator reloads the authoritative revision and rebases an older caller cursor upward before invoking the model; this allows a foreground user turn and a background Team completion to coexist without dropping either reply. The final conversation update and versioned response snapshot commit atomically, so an exact retry or process restart returns the original response without re-running the model or tools. Structured Task/Plan references survive tool replay and response replay, but streaming exposes them only in the final `Done` after commit. Raw reasoning, tool arguments, and raw tool results are not part of the public stream. Automatic Team completion is also authored by this runtime, never by Message Server or Flutter: an internal RPC accepts only an immutable completion event ID, reloads and verifies the Central-owned execution/report/artifact facts, records a bounded trusted tool observation, and asks the configured model to write the user-facing reply from the authoritative conversation history. The provider-framing prompt is not persisted. The evidence pair and generated assistant reply commit atomically, use a deterministic request identity for replay, and disable all callable tools during synthesis.
