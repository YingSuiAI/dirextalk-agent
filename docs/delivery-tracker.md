...
- [ ] Deliver Worker artifacts/logs/checkpoints through scoped S3/CloudWatch paths and deliver only deployment-declared service secrets. Typed SHA-256/size/media-bound S3 checkpoint/result claims, immutable versioned installer artifacts, KMS-encrypted upload/read-back, response-loss reconciliation, lease fencing, and untrusted persistence are complete. Direct CloudWatch delivery and declared-secret bootstrap/materialization remain fail closed, so this item is intentionally not checked.
- [ ] Persist and integrate the implemented external liveness/readiness/semantic probe engine with Deployment/Service state and events; mark Worker-local root logs as untrusted claims.
...
1. Add persistent encrypted EBS data delivery, deployment-scoped service secrets, and durable integration of the implemented external liveness/readiness/semantic probe engine so OpenClaw and knowledge-node acceptance can prove service behavior rather than only EC2 readiness.
