...
- [ ] Persist and integrate the implemented external liveness/readiness/semantic probe engine with Deployment/Service state and events; mark Worker-local root logs as untrusted claims. This requires a separately device-approved, read-back-verified ALB/TLS health route or future private VPC witness. Never infer an external target from an EIP, VPC endpoint, Worker log, or Worker-submitted URL.
...
1. Add a separately device-approved external health binding plus read-back-verified ALB/TLS health route (or a private VPC witness), then a durable monitor reconciler. It must activate only after Worker success, emit only de-sensitized evidence summaries, survive restart/replay, and never derive a target from EIP, VPC endpoint, Worker logs, or a Worker-submitted URL. EBS delivery and deployment-scoped service secrets are complete.
