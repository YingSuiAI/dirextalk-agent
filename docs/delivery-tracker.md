...
- Conversation history now omits a Cloud Worker run reference when its durable
  Execution V2 run is missing from the same conversation. Fresh login,
  restart, and second-device history loads therefore cannot restart polling for
  an already retired Worker, while valid historical results and unrelated
  references remain unchanged. Focused pure-projection and PostgreSQL history
  regressions cover valid, missing, historical, and foreign run references.
...
- Optional `agent.worker.v1` has closed owner-client list/get/destroy for the
  ...
  return an explicit correctable tool error. A pre-existing A record with a
  different IPv4 or TTL is read before Worker proxy, security-group,
  workload-state, or DNS mutation and returns a correctable `user_input` result
  with `mutation_state=unchanged`; it is never overwritten. A successful bind additionally
...
- On **2026-09-02**, Worker image release `1.1.4` upgraded the pinned Pi binary
