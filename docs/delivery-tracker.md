...
2. Add orphan manifest re-import from AWS tags/DynamoDB and explicit stale-controller alerts; exact pending/failed PostgreSQL generation replay is complete.
...
Deferred from the current release-tooling slice, but required before real acceptance: an independent Foundation-onboarding approval and dynamic per-deployment installer trust/request provisioning. The root daemon verifies and executes exact signed commands with durable replay protection, but the Worker does not yet construct those requests and its socket remains disabled.
