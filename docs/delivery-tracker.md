...
1. Complete the approved Recipe-to-Worker execution slice before spending on a release: atomically materialize the stable capability and post-claim LeaseGrant as root-owned trust, stage only digest-bound artifacts, let the compiler emit `installer.execute`, add typed copy/unpack/mount/service operations, and keep execution bounded by command, capability, and lease expiry. The implemented bridge currently accepts only root-owned artifacts already present in the AMI and the systemd socket remains disabled.
2. Add persistent encrypted EBS data delivery, deployment-scoped service secrets, and independent liveness/readiness/semantic probes so OpenClaw and knowledge-node acceptance can prove service behavior rather than only EC2 readiness.
...
Deferred from the current release-tooling slice, but required before real acceptance: an independent Foundation-onboarding approval plus production capability issuance, post-claim LeaseGrant delivery, atomic root-owned trust materialization, artifact staging, and systemd trust rotation. The Worker/root request bridge and durable replay protection are implemented, but the socket remains disabled until those AMI steps exist.
