...
- [ ] Implement retry/backoff, orphan detection by tags, `destroy_blocked`, stale-controller alerts, and re-import after Agent/DB loss. Imported provider facts now fail closed unless their mandatory plan-hash/approval-ID tags exactly re-verify against the original Worker approval or a signed Entry operation; durable discovery/re-import scheduling remains pending.
...
1. Add a dedicated multi-monitor model (or a private VPC witness) so a public-entry readiness route can coexist with an already configured liveness/semantic suite. The current path preserves the existing monitor and keeps entry activation retryable rather than replacing its suite or evidence. EBS delivery and deployment-scoped service secrets are complete.
2. Add separately typed copy/unpack/mount/service helpers where they reduce Recipe installer complexity; the current digest-locked installer artifact remains the generic, device-approved root execution path and never accepts runtime argv or shell input.
3. Add an independent device-approved Foundation onboarding transition and the release-environment resources required by the fixed AMI tool (private subnet, zero-ingress SG, versioned SSE-KMS bucket/key), then implement Foundation upgrade/full teardown and blocked-remediation workflows. Operator release credentials are not a substitute for product approval.
4. From a committed clean checkout and only after Region/cost confirmation, use the closed operator tools to publish digest-pinned Agent/Worker/Reaper artifacts, create and verify the fixed Worker AMI, import its publication, and destroy the test AMI/root snapshot with read-back. Add explicit builder EBS/ENI absence read-back and typed ECR image/repository lifecycle before claiming complete test-account cleanup.
5. Add orphan manifest re-import from AWS tags/DynamoDB, explicit stale-controller alerts, and bounded retry/backoff; exact pending/failed PostgreSQL generation replay is complete.
6. Add Managed acceptance, service operation, pairing, backup/restore, and the remaining destroy-blocked remediation workflows. Public device-approved manual destruction is complete for the current EC2/EBS/ENI/SG scope.
7. Bind private-endpoint and snapshot service-operation specs into device-signed PlanV1 before exposing those operations through dialogue/gRPC.
8. Replace the direct CloudWatch Worker-log policy: `${aws:userid}` contains `:` for assumed EC2 roles, while CloudWatch log stream names cannot use `:`. Direct Worker CloudWatch logging must not be claimed until this is redesigned.
9. Complete the remaining P3 Cloud façade, event projection, cutover preflight, and client workload views before any OpenClaw/knowledge-node acceptance claim. Restricted Cloud dialogue now binds an owner-read-back active Connection and can create only the durable research Goal; background Eino research, provider placement, three-candidate Quote, and `ready_for_confirmation` Plan are composed without approval or mutation capability.

Deferred before real acceptance: independent device-approved Foundation onboarding, immutable artifact/AMI publication in a confirmed Region, persistent volumes, service-secret delivery, probe persistence, and the remaining P3 façade/client cutover. Installer trust, artifact staging, socket activation, lease rotation, and durable replay protection are implemented locally and remain unclaimed on real AWS until that release is published and exercised.
