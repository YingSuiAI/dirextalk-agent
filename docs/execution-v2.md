...
The Agent manages at most four retained Workers for the authenticated owner and account generation. It uses the sole active AWS credential uploaded and STS-verified through the App. CPU and GPU Workers resolve the Region-local product pointers `/dirextalk/worker-images/v1/cpu/current` and `/dirextalk/worker-images/v1/gpu/current`. The referenced private AMI must belong to that exact account and carry the supported image schema, flavor, release, Pi version, and successful-test tags. A GPU AMI also carries the family set derived from its exact AWS DLAMI parent; the selected instance must be in that set. There is no fallback to Canonical or AWS public images after this contract becomes authoritative. Before pricing, both flavors resolve the AMI's real root device and snapshot minimum and place the greater of requested disk or that minimum in the confirmed ComputeSpec. Launch re-resolves the pointer and returns a deterministic fresh-quote rejection if its minimum has grown; `RunInstances` never silently raises the confirmed volume. A missing pointer and an incompatible or unverified image produce distinct maintenance guidance before any offer or cloud mutation. The Agent discovers the account's default VPC and selects only a default subnet whose availability zone currently offers the confirmed instance type. It creates an ordinary EC2 instance with an auto-assigned public IPv4. A provider client rejection is a deterministic terminal failure; only an outcome that may have committed a provider mutation remains recoverable as uncertain.
