...
The intrinsic supplies minimum vCPU, system memory, disk, estimated runtime, and an optional provider-neutral accelerator class, never an AWS instance type. GPU proposals also require a verified non-zero minimum accelerator-memory working set. For a named downloadable model, the model must first verify the exact current tag/artifact, quantization or precision, published size, runtime and accelerator/driver compatibility, context length, expected concurrency, and CPU-offload policy with available read/search tools instead of quoting from the name alone. Independent minima include model-loading peaks, KV cache or training state, runtime workspace, temporary model copies, caches, outputs, and explicit headroom. A critical unverified compatibility or capacity fact withholds a paid proposal. The closed accelerator classes are GPU, Neuron, FPGA, media, or any accelerator; omission leaves compute unconstrained. Agent reads current-generation Linux on-demand products, intersects them with actual EC2 offerings and live instance-type accelerator metadata in the Worker Region selected under the [API contract's latency-first three-Region placement policy](api-contract.md), and chooses the cheapest x86_64 shape satisfying every requirement. A fractional GPU contributes only its assigned accelerator memory, and a shape below the requested working set is ineligible. The selected concrete accelerator class, hardware name, and exact accelerator memory are persisted with the retained Worker and public plan; an accelerator request never reuses a Worker whose accelerator class is absent or incompatible. The uploaded credential's default Region is metadata, not placement authority. The plan and confirmation expose the selected exact shape, accelerator details when present, Region, and hourly compute price. Bounded jobs also expose estimated cost and maximum authorized cost; persistent services omit those two open-ended values.
...
The Agent manages at most four retained Workers for the authenticated owner and account generation. It uses the sole active AWS credential uploaded and STS-verified through the App. CPU and GPU Workers resolve exact Region/flavor AMI IDs from the Agent's embedded public release catalog. Only live-qualified publisher `066107820442` images may enter that catalog, with their schema, release/Pi versions and GPU family set derived from the exact AWS DLAMI parent. Discovery uses the customer's credentials but validates the fixed publisher owner, exact AMI ID, public visibility and supported architecture; customer SSM parameters and publisher-only AMI tags are not consumed. The selected GPU instance must be in the catalog's qualified family set. There is no fallback to generic Canonical/AWS images or newest-name discovery. Before pricing, both flavors resolve the AMI's real root device and snapshot minimum and place the greater of requested disk or that minimum in the confirmed ComputeSpec. Launch re-resolves the catalog image and returns a deterministic fresh-quote rejection if its minimum has grown; `RunInstances` never silently raises the confirmed volume. A missing catalog entry and an incompatible or unverified image produce distinct maintenance guidance before any offer or cloud mutation. Retained Workers persist their verified image ID, publisher, release/Pi versions and flavor, so publishing a new catalog entry does not invalidate already-qualified reusable Workers. The Agent discovers the account's default VPC and selects only a default subnet whose availability zone currently offers the confirmed instance type. It creates an ordinary EC2 instance with an auto-assigned public IPv4. A provider client rejection is a deterministic terminal failure; only an outcome that may have committed a provider mutation remains recoverable as uncertain.
...
The Agent connects by outbound SSH with Agent-owned key material. There is no inbound Agent callback, EIP, frontend AMI control, S3/KMS artifact path, WorkerControl service, model relay, or deploy-time Worker configuration. Worker runtime, task status, logs, and result material live under the reboot-persistent `/var/lib/dirextalk-worker` root. Image Builder preinstalls the versioned coding baseline and pinned Pi binary under `/opt/dirextalk-worker`; every task verifies the immutable image manifest and required Python, Node, Git/GitHub CLI, Go, Caddy, build-tool, and utility commands before starting, without package installation or runtime download. Recovery reads retained execution state before any new image, network, or public-egress discovery, so an unrelated discovery outage cannot mask completed remote work or result collection.
