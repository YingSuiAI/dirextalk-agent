...
The control process now shares one bounded local run budget across interactive Chat and background planning. The production defaults admit at most two simultaneous local model-backed runs, cap background planning at one of those runs, and apply a 768 MiB Go heap target without raising a stricter operator-provided `GOMEMLIMIT`. The repository Compose profile additionally caps the Agent container at 1.5 CPUs, 1 GiB memory, and 256 processes. Cloud Workers remain outside this local budget. These are the default safety boundaries for a 2 vCPU / 2 GiB host; sustained-load acceptance on the target host is still required before claiming a measured production capacity.
...
- Optional `AGENT_SEARCH_PROFILES_FILE`, a strict server-owned JSON catalog that binds each public search `profile_id` to Tavily, Brave, Exa, or Serper, that provider's exact reviewed official HTTPS endpoint, one mounted credential reference, and bounded result/timeout limits. Selecting a profile enables the native `web_search` tool. Clients and models cannot submit endpoints, credentials, methods, headers, or provider parameters.
...
- Optional `AGENT_TEAM_POLICY_FILE`, enabled only with the signed Runtime Catalog. It is a protected, strict `dirextalk.agent.team-policy/v1` document that fixes Worker/concurrency/resource/cost ceilings, safety margin, per-Worker overhead, and the allowed runtime families. Its canonical digest is signed into every Team Plan.
- Optional paired `AGENT_TEAM_MODEL_OFFER_CATALOG_FILE` and `AGENT_TEAM_COMPUTE_CATALOG_FILE`, enabled only with the Team Policy and complete AWS control configuration. The first binds trusted model prices and logical Worker credential references to immutable model Profiles; the second allowlists Regions, Availability Zones, architectures, instance types, and disk sizes. Both are protected strict JSON files. Fresh price, offering, instance-shape, quota, and capacity evidence still comes from read-only AWS APIs through the exact owner-scoped Cloud Connection.
...
The repository Compose profile defaults `AGENT_CONTAINER_CPUS` to `1.50` and `AGENT_CONTAINER_MEMORY_LIMIT` to `1024m`, leaving host capacity for PostgreSQL, networking, and operating-system work on a 2 vCPU / 2 GiB instance. Raising container or local-run limits requires a larger measured host envelope; it does not increase Cloud Worker concurrency.
...
Search configuration follows the same catalog boundary. A minimal optional catalog is `{"schema_version":1,"profiles":[{"profile_id":"tavily-default","provider":"tavily","base_url":"https://api.tavily.com/search","secret_ref":"mounted:tavily-token","max_results":10,"timeout_seconds":20}]}`. The public RuntimeService projection removes `secret_ref`; the catalog and mounted secret stay operator-controlled. Once selected, the runtime exposes only a query-only `web_search` tool. The adapter resolves the mounted key for that invocation, calls only the provider's fixed reviewed endpoint with redirects and environment proxies disabled, rejects credential-like queries, bounds the provider response to 1 MiB, and returns at most 60 KiB of normalized, de-secreted, explicitly untrusted excerpts. Tavily uses `https://api.tavily.com/search`, Brave uses `https://api.search.brave.com/res/v1/web/search`, Exa uses `https://api.exa.ai/search`, and Serper uses `https://google.serper.dev/search`; custom endpoints require a separately reviewed provider adapter rather than catalog substitution.
