...
This build is locally and fake-provider testable. AWS mutations remain fail-closed unless `AGENT_ENABLE_AWS_CONTROL=true` is explicitly configured. The release and AMI paths have not yet been executed against the authorized real ECR/EC2 environment, so this repository still does not claim a real EC2, OpenClaw, knowledge-node, persistent-data, or service-secret deployment. The Recipe compiler still emits only `worker.noop`. The Worker registry can now select `installer.execute` by `command_id` from a stable signed capability plus an exact short-lived lease grant and call the separate root daemon, but production grant issuance, root-owned trust materialization, and systemd socket activation remain deliberately disabled. This bridge currently accepts only root-owned, digest-locked artifacts already baked into the AMI. Detailed progress and prioritized gaps are tracked in [docs/delivery-tracker.md](docs/delivery-tracker.md).
