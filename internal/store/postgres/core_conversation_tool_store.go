...
func (s *CoreConversationStore) PrepareConversationTool(ctx context.Context, c core.PrepareToolCommand) (core.ToolAttempt, coretask.Task, coreconfirmation.Confirmation, error) {
	...
	// The schema intentionally has no task_id on attempts in older migrations;
	// bind through the task payload/confirmation, which are both immutable.
	...
	if _, err = tx.Exec(ctx, `UPDATE core_conversation_turns SET state='waiting_confirmation',revision=revision+1,updated_at=$2 WHERE turn_id=$1`, c.Lease.Turn.ID, now); err != nil {
		return core.ToolAttempt{}, coretask.Task{}, coreconfirmation.Confirmation{}, err
	}
