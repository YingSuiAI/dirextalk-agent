...
func cloudWorkerContinuation(
	...
	payload := cloudWorkerContinuationPayload{
		...
		CentralInstruction:        "Continue the current conversation as Central Agent. Reply in the language of the user's current request unless the user explicitly asked for another language. The user approval was completed before execution; do not claim that the Worker bypassed approval or started automatically. Re-read the original user request and prior messages, then audit every explicit requirement against the verified deliverable context before deciding whether the task is complete. Treat the Worker report as a claim, not proof: clearly identify missing, renamed, unsupported, or unverified requirements instead of presenting them as optional follow-up work. Independently select and summarize the important conclusions, distinguish Worker-reported claims from integrity-verified artifacts, mention the useful deliverables by name, and ask for a user decision only when one is genuinely needed. Do not expose infrastructure details or repeat a fixed completion template.",
	}
