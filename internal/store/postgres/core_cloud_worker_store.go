...
func (s *CloudWorkerStore) terminalExecution(ctx context.Context, supplied coretask.Task, expectedRevision uint64, terminal cloudworker.ExecutionState, code, summary string, providerResult *cloudworker.ProviderResult) (cloudworker.Execution, cloudworker.CompletionOutbox, error) {
	...
	turnUpdate, err := tx.Exec(ctx, `UPDATE core_conversation_turns SET state='accepted',response_json=NULL,
		...
		WHERE turn_id=$1 AND state='waiting_confirmation' AND revision=$4 AND cancel_requested=false`, plan.TurnID, turnSequence+2, now, turnRevision)
...
func cloudWorkerContinuation(dispatchResultRaw []byte, plan cloudworker.Plan, execution cloudworker.Execution, terminal cloudworker.ExecutionState, summary string, providerResult *cloudworker.ProviderResult, references []core.Reference) (core.ToolCall, core.ToolResult, error) {
	...
	var deliverables []cloudworker.DeliverableContext
	var omitted uint64
	if providerResult != nil {
		artifacts = make([]cloudWorkerArtifactBrief, 0, len(providerResult.Artifacts))
		for _, artifact := range providerResult.Artifacts {
			artifacts = append(artifacts, cloudWorkerArtifactBrief{ArtifactID: artifact.ArtifactID, Kind: artifact.Kind, Name: artifact.Name, MediaType: artifact.MediaType, SizeBytes: artifact.SizeBytes, SHA256: artifact.SHA256})
		}
		deliverables, omitted = providerResult.DeliverableContext, providerResult.DeliverableContextOmitted
	}
	payload := cloudWorkerContinuationPayload{Schema: "dirextalk.cloud-worker-completion/v1", ExecutionID: execution.ExecutionID, Status: string(terminal), IntegrityVerified: terminal == cloudworker.StateSucceeded, WorkerReport: summary, Artifacts: artifacts, DeliverableContext: deliverables, DeliverableContextOmitted: omitted, CentralInstruction: "Continue the current conversation as Central Agent. Use the original request and prior messages, synthesize the important conclusions, distinguish Worker-reported claims from verified artifacts, and mention useful deliverables by name."}
	...
	result := core.ToolResult{CallID: calls[0].ID, ToolName: coremodel.IntrinsicCloudWorkerProposeToolName, Content: string(content), IsError: terminal != cloudworker.StateSucceeded, RelatedTaskIDs: []string{plan.TaskID}, RelatedPlanIDs: []string{plan.PlanID}, References: references, Summary: resultSummary}
