...
const cloudWorkerLocalWorkspaceBudgetPolicyRevision uint64 = 1
...
// ResolveCloudWorkerBudgetEvidence reports only a versioned structural local
// capability limit: the Native runtime has no general workspace-archive
// executor. It never considers model arguments, congestion, timeouts, or a
...
func (s *CoreConversationStore) ResolveCloudWorkerBudgetEvidence(ctx context.Context, lease core.TurnLease) (*cloudworker.LocalBudgetEvidence, error) {
	...
	for _, attachment := range lease.Turn.AttachmentSources {
		if attachment.Kind == core.TurnAttachmentKindWorkspaceArchive {
			hasWorkspace = true
			break
		}
	}
	...
	var prompt, profileID, profileDigest string
	if err := s.pool.QueryRow(ctx, `SELECT prompt,profile_id::text,profile_snapshot_digest
		FROM core_conversation_turns WHERE turn_id=$1 AND lease_id=$2 AND lease_epoch=$3
		AND state='running' AND cancel_requested=false AND lease_expires_at>clock_timestamp()`,
		lease.Turn.ID, lease.LeaseID, lease.Epoch).Scan(&prompt, &profileID, &profileDigest); err != nil {
	...
	return newCloudWorkerWorkspaceBudgetEvidence(lease.Turn, prompt, profileID, profileDigest)
}

func newCloudWorkerWorkspaceBudgetEvidence(turn core.Turn, prompt, profileID, profileDigest string) (*cloudworker.LocalBudgetEvidence, error) {
	...
	}{"native_runtime_no_workspace_archive", cloudWorkerLocalWorkspaceBudgetPolicyRevision,
	...
	return &cloudworker.LocalBudgetEvidence{
		BudgetID: uuid.NewSHA1(uuid.NameSpaceURL, []byte("dirextalk/local-budget/native-runtime-no-workspace-archive/v1")).String(),
		Revision: cloudWorkerLocalWorkspaceBudgetPolicyRevision, Digest: hex.EncodeToString(digest[:]),
	}, nil
}
