...
func (controller *AWSController) PreviewIdentity(ctx context.Context, callerClientID, sessionID string, expectedRevision uint64, region string) (AWSIdentityEvidence, error) {
	if controller == nil || secretbootstrap.ValidateClientID(callerClientID) != nil || expectedRevision == 0 || !cloudRegionPattern.MatchString(region) {
		return AWSIdentityEvidence{}, ErrInvalid
	}
	...
	if now.IsZero() || !now.Before(descriptor.ExpiresAt) {
		return AWSIdentityEvidence{}, ErrQuoteExpired
	}
	...
	if err := controller.repository.PutAWSIdentityEvidence(ctx, evidence); err != nil {
		return AWSIdentityEvidence{}, err
	}
	persisted, err := controller.repository.GetAWSIdentityEvidence(ctx, sessionID, expectedRevision)
	if err != nil {
		return AWSIdentityEvidence{}, err
	}
	if persisted.BootstrapSessionID != sessionID || persisted.SessionRevision != expectedRevision ||
		persisted.AgentInstanceID != controller.agentInstanceID || persisted.OwnerID != descriptor.OwnerID ||
		persisted.TargetID != descriptor.TargetID || persisted.Identity != result ||
		persisted.ObservedAt.IsZero() || !persisted.ObservedAt.Before(persisted.ExpiresAt) {
		return AWSIdentityEvidence{}, ErrUnavailable
	}
