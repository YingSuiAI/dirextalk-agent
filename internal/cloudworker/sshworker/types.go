...
func (discovery Discovery) validate() error {
	...
	if err != nil || !prefix.Addr().Is4() || prefix.Bits() != 32 || !workerimage.ValidFlavor(workerimage.Flavor(discovery.ImageFlavor)) ||
		!workerimage.ValidProvenance(discovery.ImageOwnerID, discovery.ImageID, discovery.ImageVersion, discovery.ImagePiVersion) ||
...
type WorkerRecord struct {
	...
	ImageOwnerID         string             `json:"image_owner_id,omitempty"`
	ImagePiVersion       string             `json:"image_pi_version,omitempty"`
...
func (worker WorkerRecord) hasVerifiedImageContract() bool {
	...
	return flavorErr == nil && flavor == expectedFlavor && workerimage.ValidProvenance(worker.ImageOwnerID, worker.ImageID, worker.ImageVersion, worker.ImagePiVersion)
}
