...
func (p *ProposeIntrinsic) ResolveIntrinsicTools(ctx context.Context, lease coreconversation.TurnLease) ([]coreconversation.ResolvedIntrinsic, error) {
	...
	properties := map[string]any{
		...
		"min_vcpu":                   map[string]any{"type": "integer", "minimum": 1, "maximum": 128, "description": "Verified minimum virtual CPU count for runtime processes, preprocessing or tokenization, and expected concurrency."},
		"min_memory_gib":             map[string]any{"type": "integer", "minimum": 1, "maximum": 1024, "description": "Verified minimum system memory in GiB, including CPU-resident or explicitly permitted offloaded weights, model loading or conversion peaks, runtime processes, operating system, and headroom."},
		"min_accelerator_memory_gib": map[string]any{"type": "integer", "minimum": 0, "maximum": 1024, "description": "Verified minimum assigned accelerator memory in GiB, including resident weights, KV cache or training state, runtime workspace, expected concurrency, and headroom. You MUST provide a non-zero value for GPU workloads; use 0 only when accelerator_type is omitted. A fractional GPU contributes only its assigned memory, not the full physical card."},
		"disk_gib":                   map[string]any{"type": "integer", "minimum": 8, "maximum": 16384, "description": "Verified working disk in GiB for downloads, expanded or converted model copies, dependencies, runtime caches, temporary files, logs, outputs, and update or rollback headroom."},
		...
		"accelerator_type":           map[string]any{"type": "string", "enum": []any{AcceleratorGPU, AcceleratorNeuron, AcceleratorFPGA, AcceleratorMedia, AcceleratorAny}, "description": "Optional specialized accelerator requirement. Verify that the exact model artifact and runtime support the selected accelerator backend, operating system, architecture, and required driver. Use gpu for GPU workloads, neuron for AWS Trainium/Inferentia, fpga for FPGA, media for hardware media acceleration, or any only when every eligible accelerator class is verified compatible. Omit for ordinary unconstrained compute."},
