...
func (p *ProposeIntrinsic) ResolveIntrinsicTools(ctx context.Context, lease coreconversation.TurnLease) ([]coreconversation.ResolvedIntrinsic, error) {
	...
	properties := map[string]any{
		...
		"service": map[string]any{"type": "object", "description": "REQUIRED when workload_kind is service; omit for job. When the user requests a hostname, you MUST set service.hostname. Do not put AWS CLI, Route53, or DNS mutation instructions in objective; the Agent host publishes the hostname after owner confirmation.", "additionalProperties": false, "required": []any{"workload_id", "port", "health_path"}, "properties": map[string]any{
	...
	description := "Run work in a suitable retained execution environment, or propose a priced reusable environment when none is available. Use it for substantial project or shell execution, deployment, build, test, durable file delivery, long-running compute, and actual follow-up work in a retained environment. Use workload_kind=job only for finite execution and omit service. You MUST use workload_kind=service and provide service.workload_id, service.port, and service.health_path when deploying a persistent website, API, daemon, or other network endpoint. When the user requests a hostname, you MUST also provide service.hostname and MUST NOT ask the remote Worker to use AWS CLI, Route53, or another DNS API; the Agent host publishes DNS. Give estimated_runtime_minutes enough budget for setup, dependency installation, build, configuration, verification, result collection, and reasonable margin; it is not the lifetime of the retained Worker or deployed service. Do not call this tool only to inspect status; answer status questions from the live retained_worker_inventory below. The user does not need to mention cloud or remote execution. Do not use it for ordinary conversation or simple reasoning, or when the user requires local execution or forbids cloud use. Retained Worker reuse normally needs no creation confirmation. Publishing a requested hostname produces one owner confirmation for the hostname-bound plan. New resources start only after the owner reviews and confirms the offer."
