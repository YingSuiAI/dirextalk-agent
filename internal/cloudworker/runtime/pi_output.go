...
// piProcessOutputBuffer discards transient model/tool payloads while retaining
// every event needed for closed validation. Unknown or malformed lines are
// retained so the parser rejects them instead of silently hiding drift.
...
func (buffer *piProcessOutputBuffer) clone() []byte { return buffer.retained.clone() }
...
func (buffer *piProcessOutputBuffer) retainedEvent(line []byte) ([]byte, bool) {
	if len(line) == 0 {
		return nil, false
	}
	if buffer.settled {
	...
	case "tool_execution_end":
		retained = piEvent{Type: event.Type, ToolName: event.ToolName}
		if event.ToolName == PiResultToolName {
			retained.Result = event.Result
			retained.IsError = event.IsError
		}
	case "agent_end":
		retained = piEvent{Type: event.Type, WillRetry: event.WillRetry}
	case "agent_settled":
		buffer.settled = true
		retained = piEvent{Type: event.Type}
	default:
