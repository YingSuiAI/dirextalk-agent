...
func TestBuildDeliverableContextExposesFilesAndScreensSecrets(t *testing.T) {
	...
	for _, artifact := range artifacts {
		collected.Artifacts = append(collected.Artifacts, cloudresult.CollectedArtifact{Claim: cloudresult.ObjectClaim{Name: artifact.Name, MediaType: artifact.MediaType, SizeBytes: int64(len(artifact.Content))}, Content: artifact.Content})
	}
	collected.Artifacts = append(collected.Artifacts, cloudresult.CollectedArtifact{Claim: cloudresult.ObjectClaim{Name: "changes.patch", MediaType: "text/plain; charset=utf-8", SizeBytes: 22}, Content: []byte("diff --git a/a b/a\n+ok\n")})
	contextItems, omitted, err := buildDeliverableContext(collected, inputDigest, 1<<20)
	if err != nil || omitted != 0 {
		t.Fatalf("context err=%v omitted=%d", err, omitted)
	}
	...
	for _, item := range contextItems {
		...
		if strings.Contains(item.Path, "meta/") || strings.Contains(item.Path, "files/") {
			t.Fatalf("internal archive path leaked: %+v", item)
		}
	}
	if len(byPath) != 5 || byPath["README.md"].TextPreview != string(files["README.md"]) || byPath["report.csv"].MediaType != "text/csv; charset=utf-8" || byPath["preview.png"].TextPreview != "" || byPath["secret.txt"].TextPreview != "" || byPath["changes.patch"].TextPreview == "" {
		t.Fatalf("deliverable context=%+v", contextItems)
	}
}
