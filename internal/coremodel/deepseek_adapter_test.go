...
func TestDeepSeekDSMLDoesNotRewriteArgumentText(t *testing.T) {
	body := `<｜DSML｜tool_calls><｜DSML｜invoke name="save"><｜DSML｜parameter name="text" string="true"><|DSML|literal></｜DSML｜parameter></｜DSML｜invoke></｜DSML｜tool_calls>`
	calls, matched, err := parseDeepSeekDSML(body, []Tool{{Name: "save"}})
	if err != nil || !matched || len(calls) != 1 {
		t.Fatal(err)
	}
	var args map[string]string
	if json.Unmarshal([]byte(calls[0].Function.Arguments), &args) != nil || args["text"] != "<|DSML|literal>" {
		t.Fatalf("argument rewritten: %q", calls[0].Function.Arguments)
	}
}
...
func TestDeepSeekDSMLDialectRunsUnaryStreamAndContinuation(t *testing.T) {
	requests := 0
	server := httptest.NewTLSServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case 3:
			...
			_ = json.NewEncoder(w).Encode(map[string]any{"choices": []any{map[string]any{"message": map[string]any{"role": "assistant", "content": "complete"}}}})
		default:
	...
	request := CompletionRequest{Messages: []Message{{Role: RoleUser, Content: "test"}}, Tools: []Tool{{Name: "dirextalk_compatibility_echo", InputSchema: map[string]any{"type": "object"}}}}
	first, err := client.Generate(context.Background(), request)
	if err != nil || len(first.Message.ToolCalls) != 1 {
		t.Fatalf("unary=%+v err=%v", first, err)
	}
	...
	for {
		...
		calls += len(d.ToolCalls)
		if d.Content != "" {
			t.Fatal("DSML markup leaked")
		}
	}
	if progress != 2 || calls != 1 {
		t.Fatalf("progress=%d calls=%d", progress, calls)
	}
	request.Messages = append(request.Messages, first.Message, Message{Role: RoleTool, ToolCallID: first.Message.ToolCalls[0].ID, Content: "probe-ok"})
	last, err := client.Generate(context.Background(), request)
	if err != nil || last.Message.Content != "complete" || requests != 3 {
		t.Fatalf("continuation=%+v requests=%d err=%v", last, requests, err)
	}
}
...
func TestDeepSeekDSMLStreamReturnsFormatErrorWithoutLeakingMarkup(t *testing.T) {
	...
	if delta, err := stream.Recv(); err != nil || !delta.ProviderProgress || delta.Content != "" {
		t.Fatal("private stream progress missing")
	}
