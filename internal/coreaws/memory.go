...
func NewMemoryRepository() *MemoryRepository {
	return &MemoryRepository{credentials: map[string]Credentials{}, credentialRevisions: map[string]map[int64]Credentials{}, disabledCredentials: map[string]time.Time{}, replays: map[string]memoryReplay{}, testClaims: map[string]memoryCredentialTestClaim{}}
}

func replayKey(op, key string) string { return op + ":" + key }

// ReplayCredential exposes the durable idempotency snapshot for restart and
// recovery tests without exposing credential secret bytes.
