...
func NewCoreExecutionV2Service(domain *coreexecutionv2.Service, owner func(context.Context) string) (*CoreExecutionV2Service, error) {
	if domain == nil || owner == nil {
		return nil, coreexecutionv2.ErrInvalid
	}
	return &CoreExecutionV2Service{Domain: domain, Owner: owner}, nil
}

func (s *CoreExecutionV2Service) owner(ctx context.Context) (string, error) {
	if s == nil || s.Domain == nil || s.Owner == nil {
		return "", coreexecutionv2.ErrNotReady
	}
	owner := strings.TrimSpace(s.Owner(ctx))
	if owner == "" {
		return "", status.Error(codes.PermissionDenied, "authenticated owner is required")
	}
	return owner, nil
}
...
func (s *CoreExecutionV2Service) Execute(ctx context.Context, req *agentv1.CoreExecutionV2ExecuteRequest) (*agentv1.CoreExecutionV2ExecuteResponse, error) {
	...
	result, err := s.Domain.Handle(ctx, owner, req.GetAction(), input)
	if err != nil {
...
func (s *CoreExecutionV2Service) Get(ctx context.Context, req *agentv1.CoreExecutionV2GetRequest) (*agentv1.CoreExecutionV2GetResponse, error) {
	...
	record, err := s.Domain.Get(ctx, owner, req.GetKind(), req.GetId(), req.GetRevision())
	if err != nil {
		return nil, mapExecutionError(err)
	}
...
func (s *CoreExecutionV2Service) ListEvents(ctx context.Context, req *agentv1.CoreExecutionV2ListEventsRequest) (*agentv1.CoreExecutionV2ListEventsResponse, error) {
	...
	events, next, err := s.Domain.Events(ctx, owner, req.GetKind(), req.GetId(), req.GetAfterSequence(), limit)
	if err != nil {
		return nil, mapExecutionError(err)
	}
...
func mapExecutionError(err error) error {
	...
	case errors.Is(err, coreexecutionv2.ErrConflict):
