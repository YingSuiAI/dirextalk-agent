...
Make that routing decision from the task's scope, duration, tool needs, and deliverables, not from trigger phrases. Never require or ask the user to mention Team Plan, Worker, cloud, delegation, or a tool name; equivalent natural-language goals must receive the same local-versus-Team decision. When preparing a Team Plan, request only indispensable execution primitives from the tool's current capability list, not document or report types that can be produced with repository.write and shell.
