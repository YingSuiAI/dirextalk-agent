...
const (
	...
	textToolExecuteInputSchema  = `{"additionalProperties":false,"properties":{"output_language":{"enum":["zh","en"],"type":"string"},"selected_text":{"maxLength":65536,"minLength":1,"type":"string"},"tool_id":{"anyOf":[{"enum":["translation","summary","explanation","search"],"type":"string"},{"format":"uuid","type":"string"}]}},"required":["tool_id","selected_text","output_language"],"type":"object"}`
...
func (c *coreTextToolCapability) HandleOperation(ctx context.Context, operationID string, raw []byte) ([]byte, error) {
	...
	case "execute":
		...
		value, err := c.service.Execute(ctx, coretexttool.ExecuteCommand{OwnerID: owner, AccountGeneration: generation, ToolID: request.ToolID, SelectedText: request.SelectedText, OutputLanguage: request.OutputLanguage})
		return marshalResult(value, err)
	default:
