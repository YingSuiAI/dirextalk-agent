...
The local sandbox is limited to 30 CPU seconds, 256 MiB memory, 32 processes, 16 MiB total files, and no network. Use cloud_worker_run on an existing Worker for required network or execution unavailable from specialized tools; use cloud_worker_propose only when a new machine is needed for that work, including repository cloning, dependency installation, builds, deployments, long-running services, durable remote work, or work exceeding those limits. Skip a local attempt that cannot satisfy the task. If the user asks only for a conceptual plan or forbids starting Worker work, answer directly without calling the tool. Follow-up work requiring a Worker's workspace or artifacts must use cloud_worker_run on that retained Worker; local_sandbox_run cannot access the Worker filesystem. Routine changes, optimization, configuration edits and application restarts are maintenance jobs, not new service deployments. Infer the existing Worker from the user's domain/server/service or recent task and current inventory; do not ask the user to supply an internal ID when the target is clear. cloud_worker_run never creates a machine and preserves existing service registration and platform-managed networking. Only cloud_worker_propose requests a new machine, for an explicitly needed new environment; never switch to it merely because an existing Worker is busy or unavailable.

GitHub MCP is the direct path for advertised reads and only the lightweight writes mcp__github__issue_write, mcp__github__add_issue_comment, and mcp__github__merge_pull_request. Any repository code workflow—including cloning a public or private repository, creating a branch or ref, changing or deleting files, installing dependencies, editing, testing, committing, pushing, or creating or updating a code pull request—must execute on a Worker using the task-scoped GitHub credential: cloud_worker_run for an existing target, cloud_worker_propose for a new machine. Only new machine creation waits for owner confirmation. Do not claim that GitHub code changes are unavailable merely because direct MCP file, ref, or pull-request creation tools are intentionally absent.
...
func containsCloudWorkerIntrinsic(tools []ResolvedIntrinsic) bool {
	for _, tool := range tools {
		if coremodel.IsCloudWorkerExecutionTool(tool.Tool.Name) {
			return true
		}
	}
	return false
