...
func (s *Service) resolveIntrinsicTools(ctx context.Context, lease TurnLease) ([]ResolvedIntrinsic, error) {
	if lease.Turn.GroupOrigin != nil {
		...
		if schedules, ok := s.turns.(ConversationScheduleStore); ok &&
			strings.TrimSpace(lease.Turn.OwnerID) != "" && lease.Turn.AccountGeneration != 0 {
			return s.appendGroupIntrinsic(tools, groupScheduleIntrinsic(schedules, lease), origin)
		}
		return tools, nil
	}
	...
	if sites, ok := s.turns.(ConversationStaticSiteStore); ok && s.staticSites != nil && strings.TrimSpace(lease.Turn.OwnerID) != "" && lease.Turn.AccountGeneration != 0 {
		tools = append(tools, staticSiteReadIntrinsic(sites, s.staticSites, lease))
